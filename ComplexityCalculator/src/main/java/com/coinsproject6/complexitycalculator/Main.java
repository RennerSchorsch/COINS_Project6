...
public class Main {
    ...
    private static boolean zeilenDokumente = true;
        ...
        try {
            ...
            if (zeilenDokumente) {
                ...
                for (String tmp : content) {
                    ...
                    Text tweet = analyzer.analyzeText(tmp);

                    if (!tweet.getTextContent().isEmpty()) {
                        tweets.add(tweet);
                    }
                }

                logger.debug("Anzahl der Tweets nach der Analyse: " + tweets.size());
            ...
            logger.error("Fehler beim analysieren des Textes. NullPointer." + e.getMessage());
        }
    }
